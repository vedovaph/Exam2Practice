...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        # ---------------------------------------------------------
        string1 = ''
        empty = self.volume - len(self.contents)
        if len(additional_contents) < empty:
            for k in range(len(additional_contents)):
                self.contents = self.contents + additional_contents[k]
        if len(additional_contents) >= empty:
            for k in range(empty):
                self.contents = self.contents + additional_contents[k]
        for k in range(empty, len(additional_contents)):
            string1 = string1 + additional_contents[k]
        return string1
